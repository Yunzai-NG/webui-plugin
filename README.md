...
装依赖与构建由内核代跑：索引里本包声明了 `setup.scripts: ["build"]`，故市场会在包目录内
依次执行 `pnpm install` 与 `pnpm run build`。这一步不能省 —— 本仓库不发布至 npm，git 里
也不含构建产物，没有那次 `build` 就没有 `dist/index.js`。下面「从源码」那两步手工做的正是
同一件事。
