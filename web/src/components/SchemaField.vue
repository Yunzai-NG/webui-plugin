 ...
 * 依赖方向：依赖 AppIcon 与类型
 * 生命周期：随父表单
 * 注意事项：这里是整个面板中唯一决定控件形态的位置，全部形态来自 `s.*` 声明推导出的描述结构体。
 *          六条约定：
 *          1) `path` 以点号连接，与 `issues[].path` 一致 —— 400 响应据此逐字段标红
 *          2) `showWhen` 在此处判断而非由父组件过滤：判断要用同级字段的当前值，只有这一层知道其范围
 *          3) 行形制是「左：图标 + 标签 + 说明；右：控件」，配置页动辄二十余项，堆叠式放不下
 *          4) 图标由 widget 推导、不承载信息，故一律 `aria-hidden`，去掉不丢内容
 *          5) 多行文本与标签列表整行铺开，不进 260px 的右列
 *          6) 下拉与分段回传枚举项的**原值**：`<select>` 的 value 恒为字符串，直接回传会把数值项
 *             写成 `"3001"` 而校验失败，故按字符串形态反查候选项
 */
...
/**
 * 控件类型 → 图标，一律 24×24 网格上的 path 数据
 *
 * 刻意不标注 `Record<string, string>`：那样键退化为 `string`，在
 * `noUncheckedIndexedAccess` 之下取值成 `string | undefined`，取图标处要多写一次
 * 恒不触发的兜底。不标注则键为字面量联合。
 */
const ICONS = {
  /** 单行文本：竖排的 I 形光标 */
 ...
 * 把正在输入的那一项收进数组
 *
 * 空白与重复静默收起，不弹错。回车后不收起输入框：连着添几项是常态，失焦那一路才收起。
 */
...
/**
 * 移除第 n 项；按序号而非按值 —— 数组允许有重复项，按值删会一次删掉两枚
 * @param index 项的序号
 */
...
/** 约束说明的静态部分，放左列标签之下：右列只有 260px，且 `block` 类控件已横跨整行 */
 ...
 * 打开目录选择器
 *
 * 手敲输入框一并保留：列目录接口在只读模式下停用，那时这个按钮只有 403，而配置项仍须可填。
 * 起始目录不在此算 —— 选目录与选文件的起点不同，而选择器自己知道 mode。
 */
...
/**
 * 从多行文本还原数组；空行一律丢弃 —— 一个空前缀项会让所有消息都匹配
 * @param text 文本
 */
...
/**
 * 从输入框还原数字；空串还原成 undefined 而非 0 —— 后者往往是另一个合法取值（超时 0 = 不超时）
 * @param text 文本
 */
 ...
 * 提交时长
 *
 * 空串与非数字一概不提交（`joinDuration` 给 undefined）：这里的字段本来就有值，按 0 提交
 * 等于替使用者改了配置。账号页的重连覆盖那侧相反（留空即跟随全局），故那个判断归调用方。
 * 改单位时数值原样保留，不做等值换算 —— 换算必有一半场合猜错，且错得静默。
 * @param text 数值输入框的内容
 * @param unit 单位
 */
