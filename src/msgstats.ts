 ...
 * 注意事项：与 IO 分家是为了能测 —— 判天界、跨天翻桶、裁剪、聚合算错了也照样跑，只是数字偏了、
 *          没有报错。IO 那一半在 `msgcollect.ts`。三条约定：
 *          1) 天界按本地时区切而非 UTC（使用者问的是他墙上的今天），故 `dayKey` 只在写入时求一次
 *          2) 发送侧的 `sticker` 恒为 0：内核 `message/sent` 的 `kinds` 是按段 `type` 聚合的计数，
 *             而「表情图」是 `image` 段的 `subType === 1`，那层信息在聚合时已丢。界面须就此说明
 *          3) 收侧拿得到完整 `Segment[]`，故三项齐全
 */
 ...
 * 一个会话在某一天的收发计数
 *
 * 总量与 `by`（按账号细分）并存，不把账号并进 `peer` 键：并进去会让同一个群因两个账号
 * 在里面而裂成两行，「这个群一共多少条」就问不出来了。
 */
 ...
 * 某一小时的收发计数
 *
 * 刻意不按会话细分：那样一天就是 `24 × 会话数` 个桶，而趋势图问的是总量。
 * 会话维度由 `PeerDay` 那一层给。
 */
...
/** 小时桶保留多少天：按小时看的只有「当日」与「1 日」两档，全存 40 天则桶数是 24 倍且读不到 */
export const HOUR_KEEP_DAYS = 7

/**
 * 求某个时刻的本地日期键
 *
 * 手工取年月日而非 `toISOString().slice(0, 10)`：后者按 UTC 切，东八区凌晨 8 点前的
 * 消息会被记到前一天。
 * @param at 毫秒时间戳
 * @returns `YYYY-MM-DD`
 */
 ...
 * 拼一个账号的键
 *
 * 用 `platform:account` 而非把适配器 id 也拼进去：同一个账号换适配器接入时使用者眼里
 * 仍是同一个号，历史不该断成两截。适配器 id 只作显示用，存在 `AccountRef.adapter` 里。
 * @param ref 账号身份
 * @returns 键
 */
...
export class StatsBuckets {
   ...
   *
   * 修盘上那份旧数据：发侧一度用记录 uuid 作键、收侧用平台 id，同一个号成了两个账号。
   * 合并而非丢弃 —— uuid 那侧的计数是真实发生过的发送量。改写三处，与 `add` 写入的三处
   * 一一对应（账号身份表、会话日的 `by`、小时槽的 `by`）；漏掉任一处只会让数字对不上，不报错。
   ...
   * 丢掉超出保留期的天
   *
   * 按日期字符串比较（`YYYY-MM-DD` 的字典序即时间序）而非算时间差：后者要处理夏令时与闰秒。
  ...
  prune(now: number): number {
    ...
    for (const day of [...this.#days.keys()]) {
      ...
      // 过了小时保留期就摘掉 `hours`、天汇总留着：小时桶是天汇总的 24 倍，
      // 而 7 天以外没有任何一档界面会读它
  ...
  /**
   * 取一份可落盘、可直接送给前端的快照，顺带清掉 dirty 标记
   * @returns 文档
   */
  snapshot(): StatsDoc {
    this.#dirty = false
 ...
 * 校验一份读回来的文档
 *
 * 磁盘那份按不可信输入对待，逐层校验而非只看 `version`：结构错位会让界面画出天文数字，
 * 那比「统计从零开始」难查得多。
 * @param raw 已解析的 JSON
...
export function parseStatsDoc(raw: unknown): StatsDoc | undefined {
  ...
  // 只认 2：版本 1 没有账号维度，迁过来只能挂到一个「未知账号」名下，
  // 此后按账号筛选会长期错着 —— 从空开始更诚实
